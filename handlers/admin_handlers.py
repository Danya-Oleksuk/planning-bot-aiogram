...
async def admin_panel(message: Message, state: FSMContext, user_repo):
    ...
    if is_admin(message.from_user.id):
        await message.answer(
            "👨🏻‍💻 Админ панель активирована:", reply_markup=markup.admin_panel
        )
    else:
        await message.answer(
            "🤷🏻 Непонятная команда, попробуйте снова",
...
async def show_vip_count(message: Message, state: FSMContext, user_repo, vip_repo):
    ...
    if is_admin(message.from_user.id):
        ...
        await message.answer(
            f"{len(vip_users)} vip пользователей", parse_mode=ParseMode.MARKDOWN
        )
    else:
        await message.answer(
            "🤷🏻 Непонятная команда, попробуйте снова",
...
async def show_non_vip_count(message: Message, state: FSMContext, user_repo, vip_repo):
    ...
    if is_admin(message.from_user.id):
        ...
        await message.answer(
            f"{len(not_vip_users)} не vip пользователей", parse_mode=ParseMode.MARKDOWN
        )
    else:
        await message.answer(
            "🤷🏻 Непонятная команда, попробуйте снова",
...
async def fsm_state_for_user_ban(message: Message, state: FSMContext, user_repo):
    ...
    try:
        ...
        if not await user_repo.is_user_in_database(
            telegram_id=user_id
        ) or await user_repo.get_user_is_banned(user_id):
            raise ValueError

        if await user_repo.get_user_is_banned_by_admin(user_id):
            await message.answer(
                f"⚠️ Пользователь {user_id} уже был забанен.",
                reply_markup=markup.admin_panel,
            )
        else:
            ...
            if user_ban:
                await send_user_message(
                    user_id, "🚫 Вы были забанены администратором бота."
                )
                await message.answer(
        ...
        await message.answer(
            "⚠️ Был введен <b>не правильный id</b> или <b>юзер забанил бота</b>",
    ...
    try:
        ...
        if await user_repo.get_user_is_banned_by_admin(user_id):
            ...
            if user_unban:
                await send_user_message(
                    user_id,
                    "🔓 Вы были разбанены администратором бота.",
                    keyboard=markup.get_menu(user_id),
                )
                await message.answer(
            ...
            await message.answer(
                f"⚠️ Пользователь {user_id} уже разбанен.",
                reply_markup=markup.admin_panel,
            )
        await state.clear()
    except ValueError:
...
async def post_text_regular(message: Message, state: FSMContext, user_repo, vip_repo):
    ...
    try:
        if data["until_date"] in ("1w", "1m", "1y", "forever"):
            ...
            await vip_repo.activate_vip(
                user_id=int(data["user_name"]), until=until_date
            )

            await message.answer(
                "🥳 Vip статус успешно подарен",
                reply_markup=markup.get_menu(message.from_user.id),
            )
            ...
            await message.answer(
                "⚠️ <b>Дата</b> введена не правильно",
                parse_mode=ParseMode.HTML,
                reply_markup=markup.admin_panel,
            )
    except Exception as ex:
        await message.answer(
            f"⚠️ <b>Не получилось подарить vip, ошибка</b> - {ex}",
            parse_mode=ParseMode.HTML,
            reply_markup=markup.get_menu(message.from_user.id),
        )
    finally:
...
async def post_picture(message: Message, state: FSMContext, user_repo):
    ...
    await message.answer_photo(
        ...
        caption=data["text"],
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=markup.get_post_confirm(),
    )
...
async def is_post_confirm(
    ...
    if callback_query.data == "post_confirm":
        ...
        await state.clear()
        await callback_query.message.answer(
            "😞 Рассылка была отменена", reply_markup=markup.admin_panel
        )
        await callback_query.answer()
