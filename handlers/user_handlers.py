...
async def start(message: Message, state: FSMContext, user_repo):
    ...
    if await user_repo.is_user_in_database(telegram_id=message.from_user.id):
        ...
        await message.answer(
            "👋 Привет, вы зарегистрировались, удачи в планировании!",
            reply_markup=markup.get_menu(message.from_user.id),
        )
...
async def profile(message: Message, state: FSMContext, user_repo):
    ...
    await message.answer(
        "<b>Твои данные для логина на сайте:</b>\n\n"
...
async def show_plan(
    message: Message, state: FSMContext, vip_repo, user_repo, task_repo
):
    if not await check_and_notify_registration(message, user_repo):
        return
    ...
    if not tasks:
        await message.answer(
            "❗️Ваш план на сегодня пуст!",
            reply_markup=markup.get_menu(message.from_user.id),
        )
    else:
        if await vip_repo.is_user_vip(user_id=message.from_user.id) and not is_admin(
            message.from_user.id
        ):
            is_still_vip = datetime.datetime.now() < await vip_repo.get_vip_expiration(
                message.from_user.id
            )
            if not is_still_vip:
                await vip_repo.deactivate_vip(user_id=message.from_user.id)

        if await vip_repo.is_user_vip(user_id=message.from_user.id) or is_admin(
            message.from_user.id
        ):
            completed_tasks = []
            not_completed_tasks = []
            ...
            if not completed_tasks:
                answer = "🗂 <b>Твой план:</b>\n\n" + "\n".join(
                    [
                        f"{i + 1}. {list(task.keys())[0]} - {list(task.values())[0]}"
                        for i, task in enumerate(not_completed_tasks)
                    ]
                )
                ...
                answer = (
                    ...
                    + "\n".join(
                        [
                            f"{i + 1}. <s>{list(task.keys())[0]}</s> - {list(task.values())[0]}"
...
async def clear_plan(message: Message, state: FSMContext, user_repo, task_repo):
    ...
    if res is True:
        await message.answer(
            "❗️Теперь ваш план пуст", reply_markup=markup.get_menu(message.from_user.id)
        )
    else:
...
async def create_task_(
    ...
    if await task_repo.count_user_tasks(user_id=message.from_user.id) < 3:
        await message.answer(
            "✍️ Введите название задачи:", reply_markup=ReplyKeyboardRemove()
        )
        ...
        if is_still_vip:
            ...
            await vip_repo.deactivate_vip(user_id=message.from_user.id)
            await message.answer(
                "✍️ К сожалению ваша подписка прошла, продлить - /pay",
                reply_markup=markup.edit_menu,
            )
    else:
        await message.answer(
            "✍️ К сожалению лимит исчерпан, /pay", reply_markup=markup.edit_menu
        )


@router_2.message(F.text.in_(["❌ Удалить задачу", "/remove_task"]))
async def initiate_task_removal(
    message: Message, state: FSMContext, user_repo, task_repo
):
    if not await check_and_notify_registration(message, user_repo):
        return
    ...
    if not tasks:
        ...
        await message.answer(
            "Выберете задачу, которую вы хотите удалить:\n\n"
            + "\n".join(
                [f"{i + 1}. {task[0]} - {task[1]}" for i, task in enumerate(task_pairs)]
            ),
            reply_markup=markup.inline_builder(
                num=await task_repo.count_user_tasks(user_id=message.from_user.id),
                emoji="🗑",
                action="delete",
            ),
        )
...
async def edit_task_status_(message: Message, state: FSMContext, user_repo, task_repo):
    ...
    if not tasks:
        ...
        tasks = [
            f"{i + 1}. {task_id} - {status}"
            for i, task in enumerate(tasks)
            for task_id, status in task.items()
            if status == "❌"
        ]
        if not tasks:
            ...
            await message.answer(
                "Выберете задачу, которую вы хотите выполнить:\n\n"
...
async def pay(message: Message, state: FSMContext, vip_repo, user_repo):
    ...
    if is_admin(message.from_user.id):
        await message.answer(
            "👨🏻‍💻 Ты и так админ", reply_markup=markup.get_menu(message.from_user.id)
        )
    else:
        ...
        if vip_until_date is None:
            await message.answer(
                "<b>Приобретая премиум, вы открываете:</b>"
            ...
            if vip_until_date > datetime.datetime.now() + datetime.timedelta(days=999):
                await message.answer(
                    "<b>Ваша подписка активна навсегда ✅</b>"
                ...
                await message.answer(
                    f"<b><u>Ваша подписка еще активна до {vip_until_date.strftime('%Y-%m-%d')}</u></b>"
                    "\n\n📌 <i>Вы открыли доступ ко всем функциям!</i>"
...
async def task_name(message: Message, state: FSMContext, stats_repo, task_repo):
    if not message.text:
        ...
        await message.answer(
            "❗️Название задачи не должно быть больше 50 символов\n\n✍️  Введите название задачи:"
        )
        return

    await task_repo.create_task(
        user_id=message.from_user.id, task_description=message.text
    )
...
async def confirm_task_removal(call: CallbackQuery, task_repo):
    ...
    result = await task_repo.delete_task_by_index(
        number_of_task=task_num, user_id=call.from_user.id
    )
        ...
        if not tasks:
            ...
            await call.message.answer(
                ...
                + "\n".join(
                    [
                        f"{i + 1}. {task[0]} - {task[1]}"
                        for i, task in enumerate(task_pairs)
                    ]
                ),
                reply_markup=markup.inline_builder(
                    num=await task_repo.count_user_tasks(user_id=call.from_user.id),
                    emoji="🗑",
                    action="delete",
                ),
            )
...
async def update_task_status(call: CallbackQuery, stats_repo, task_repo):
    task_num = int(call.data.split("_")[1])
    result = await task_repo.mark_task_completed(
        user_id=call.from_user.id, task_number=task_num
    )
    ...
    if result is True:
        ...
        if not tasks:
            await call.message.answer(
                "❗️Ваш план теперь пуст",
                reply_markup=markup.get_menu(call.from_user.id),
            )
            return
...
async def process_successful_payment(message: Message, vip_repo):
    ...
    await message.answer(
        "🥳 Спасибо за поддержку бота. Все услуги предоставлены!"
